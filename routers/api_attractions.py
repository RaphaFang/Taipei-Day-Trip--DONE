...
from utils.datamodel import AttractionSearch

router = APIRouter()
headers = {"Content-Type": "application/json; charset=utf-8"}

@router.get("/api/attractions")
async def api_attractions(request: Request, input: AttractionSearch):
    try:
        db_pool = request.state.db_pool.get("basic_db") 
        with db_pool.get_connection() as connection:
            with connection.cursor(dictionary=True) as cursor:
                offset_num = input.page*12
                keyword_format = f"%{input.keyword}%" 
                ...
                next_page = input.page+1 if sum_rows['FOUND_ROWS()'] > (input.page+1)*12 else None
